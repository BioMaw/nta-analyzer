...
def parse_file(file):
    ...
    df = df.drop(
        columns=["Number", "Volume / nm^3", "Area / nm^2", "Concentration / cm-3"]
    )
    return df
...
def plot_bars(df, export=False):
    ...
    for i, cell_line in enumerate(cell_lines):
        ...
        if np.ndim(axes) == 2:
            ...
            ax = axes if len(cell_lines) == 1 else axes[i]
    ...
    if export:
        fig.savefig(
            Path(export, figtitle + " .png"), format="png", dpi=300, bbox_inches="tight"
        )
...
def plot_hist(df, binwidth=5, export=False, binexport=False):
    ...
    )  # Set the style to "ticks" and specify font_scale value
    ...
    # Add overall title and axis labels
    figtitle = (
        "Histograms by cell line and treatment. Binsize = " + str(binwidth) + " nm"
    )
    plt.suptitle(figtitle)
    ...
    if binexport:
        ...
        bins = np.arange(
            p_data["Size / nm"].min(), p_data["Size / nm"].max() + binwidth, binwidth
        )
        counts, bin_edges = np.histogram(
            p_data["Size / nm"], bins=bins, weights=p_data["conc_corr_norm"]
        )
        ...
        bin_data = pd.DataFrame(
            {"bin_center": bin_centers, "bin_magnitude": bin_magnitudes}
        )
...
if len(xlsx_list) != 0:
    # load the description of the tubes from excel file
    for file in xlsx_list:
        df_desc = pd.read_excel(file)

    # merge on ID and drop all the redundant columns
...
sum_conc = df_master.groupby(["biol_replic", "tech_replic", "treatment"])[
    "conc_corr"
].transform("sum")
...
unique_sizes = df_master.groupby(["biol_replic", "tech_replic", "treatment"])[
    "conc_corr"
].apply(count_unique_nonzero)
...
mean_df = df_avg.groupby(["biol_replic", "treatment"])[
    ["Size / nm", "conc_corr_norm"]
].apply(
...
df_count_stat["mean"] = df_count_EV.groupby(["biol_replic", "treatment"])[
    "EV/Cellcount"
].mean()
df_count_stat["std"] = df_count_EV.groupby(["biol_replic", "treatment"])[
    "EV/Cellcount"
].std()
...
df_pooled = (
    df_avg.groupby(["biol_replic", "Size / nm", "treatment"])["conc_corr_norm"]
    .mean()
    .reset_index()
)
